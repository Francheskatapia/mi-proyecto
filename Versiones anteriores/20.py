...
def nmap_scan(ip, mode="syn"):
    ...
    # Configurar el comando según el modo
    if mode == "syn":
        comando = ["nmap", "-sS", "-p-", ip]  # Escaneo SYN
    elif mode == "connect":
        comando = ["nmap", "-sT", "-p-", ip]  # Escaneo TCP Connect
    elif mode == "udp":
        comando = ["nmap", "-sU", "-p-", ip]  # Escaneo UDP
    elif mode == "version":
        comando = ["nmap", "-sV", ip]  # Detección de versiones
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ip", required=True, help="Dirección IP base a escanear (Ejemplo: 192.168.1.1)")
    ...
    # Realizar ping sweep si se proporciona la máscara
    if args.mascara:
        try:
    ...
    # se implemento nmap, para escaneo de puerto TCP y SYNC, al igual que argparse nuevamente, para ingresar por linea de comandos (opcional)
    # y se le agrego un timeout a nmap, para evitar que el escaneo tarde mucho tiempo